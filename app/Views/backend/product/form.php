...
<div class="row">
    <div class="col-md-12 " style="padding-right: 2rem!important; padding-bottom: 10px;">
        <h4 class="m-b-lg"><?=(!$finditem)?display('productlist'):"update"?></h4>
        ...
        <div class="widget">
            ...
            <div class="widget-body">
                <form action="<?php echo base_url('admin/product/form'); ?>" method="post"
                    ...
                    <?= csrf_field() ?>
                    <div class="form-group  <?= ($validation->getError('title')) ? 'has-warning' : '' ?>">
                        <label><?php echo display('title') ?></label>
                        <input type="text" class="form-control  " id="title" name="title" value="<?=($finditem)?$finditem->title:''?>"
                               style=" <?= ($validation->getError('title') ? 'border-color: #dc3545;' : '') ?>"
                               placeholder="<?= ($validation->getError('title')) ? display('holdertitlerequired') : display('holdertitle') ?>">

                    </div>
                    <div class="form-group">
                        <label><?php echo display('description') ?></label>
                        <textarea name="decription" class="m-0" data-plugin="summernote"
                                  data-options="{height: 250}"><?=($finditem)?$finditem->description:''?></textarea>
                    </div>
                    <button type="submit"
                            class="btn btn-primary btn-md btn-outline"><?=($finditem)?display('update'):display('submit')?></button>
